...
output = json.dumps({'seed': seed, 'context': ex, 'questions': []})
print(output)
`
};
...
ui32 = np.iinfo(np.uint32)
seed = np.random.randint(1, ui32.max)
df = pd.read_csv("https://vincentarelbundock.github.io/Rdatasets/csv/datasets/trees.csv")
x = np.round(df['Volume'].sample(df.shape[0], replace=True, random_state=seed).mean())
ex = 'Do something with {}.'.format(x)
output = json.dumps({'seed': seed, 'context': ex, 'questions': []})
print(output)
`
};
...
output = json.dumps({'seed': None,
'context': "The majority of this class focused on Normal linear models.",
'questions': ["What about them is Normal?", "What about them is linear?  For full points, you must be very specific about what exactly is linear.", "Is it possible for normal linear models to fit curves to data?  Explain."]})
print(output)
`
};
